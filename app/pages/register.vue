...
<template>
  <div class="max-w-md mx-auto mt-12">
    <UCard>
      <template #header>
        <h1 class="text-xl font-bold text-center">{{ t('auth.registerTitle') }}</h1>
      </template>
      ...
      <form @submit.prevent="handleRegister" class="space-y-4">
        ...
        <UFormField :label="t('auth.name')">
          <UInput v-model="name" :placeholder="t('auth.name')" required class="w-full" />
        </UFormField>
      ...
      <template #footer>
        <p class="text-center text-sm text-(--ui-text-muted)">
          {{ t('auth.hasAccount') }}
          <ULink :to="localePath('/login')" class="text-(--ui-primary) hover:underline">
            {{ t('auth.login') }}
          </ULink>
        </p>
      </template>
